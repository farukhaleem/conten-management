...
let LivePreview = (props) => {
	...
	if (typeof (props.costPerPage[0]) === 'undefined') {
		return (
		...
		<Card className={classes.paperItem}>
			<CardContent>
				...
				<List className={classes.list}>
					...
					<PreviewListItem itemTitle="Cost per page" itemData={`${parseFloat(Number(props.costPerPage[0]) * props.currencyRate[0]).toFixed(2)} ${props.state[0].currency}`} style={{ fontWeight: 500 }} />
					<PreviewListItem itemTitle="Assignment Cost" itemData={`${parseFloat(props.state[0].pages * Number(props.costPerPage[0]) * props.currencyRate[0]).toFixed(2)} ${props.state[0].currency}`} style={{ fontWeight: 500 }} />

					<PreviewListItem
						itemTitle="VAT"
						itemData={
							`${parseFloat((props.state[0].pages * Number(props.costPerPage[0]) * props.currencyRate[0]) / 100 * 20).toFixed(2)} ${props.state[0].currency}`
						}
						style={{
							color: 'red',
							fontWeight: 500
					...
					<PreviewListItem
						itemTitle="Discount"
						itemData={
							`${parseFloat((props.state[0].pages * Number(props.costPerPage[0]) * props.currencyRate[0]) / 100 * 19).toFixed(2)} ${props.state[0].currency}`
						}
						style={{
							color: '#32E340',
							fontWeight: 500
					...
					<PreviewListItem
						...
						itemData={
							`${parseFloat((props.state[0].pages * Number(props.costPerPage[0]) * props.currencyRate[0]) / 100 * 101).toFixed(2)} ${props.state[0].currency}`
						}
						style={{
							fontWeight: 700
						}}
